...
describe("AuthenticatorFacade", () => {
  ...
  it("deberia retornar un token", () => {
    expect(
      data.authenticatorFacade.getToken("Bearer cn389ncoiwuencr")
    ).to.be.equals("cn389ncoiwuencr");
    //
    try {
      data.authenticatorFacade.getToken("");
    } catch (e) {
      expect(e.msg.toString()).to.be.equals(
        "[credentials_required] Authentication is mandatory"
      );
    }
    //
    try {
      ...
      expect(e.msg.toString()).to.be.equals(
        "[bad_format] Format must be: <type> <credentials>"
      );
    }
    //
    try {
      ...
      expect(e.msg.toString()).to.be.equals(
        "[credentials_bad_scheme] Authentication type not allowed"
      );
    }
    //
    try {
  ...
  it("Deberia autenticar incorrectamente", () => {
    data.authenticatorFacade.on(
      data.authenticatorFacade.ON_BAD_AUTHENTICATION,
      (auuht, msg) => {
        expect(msg.toString()).to.be.equals(
  ...
  it("Deberia autenticar correctamente", () => {
    data.authenticatorFacade.on(
      data.authenticatorFacade.ON_AUTHENTICATION,
      (resource, signed) => {}
    );
  ...
  it("Deberia verificar si es o no valido un token", () => {
    ...
    const isAuth = data.authenticatorFacade.isAuthenticated(
      "Bearer " + gooodTOken
    );
    expect(isAuth).to.be.true;
    const badAuth = data.authenticatorFacade.isAuthenticated(
      "Bearer " + data.badToken
    );
    expect(badAuth).to.be.false;
  });
});
