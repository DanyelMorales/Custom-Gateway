...
import loadJson = require("../JSON");
import {JSONType} from '@utils/JSONUtils';

const fixture = __dirname + "/config";
describe('JSON WRAPPER', () => {

  let reader: any;
  let wrapper: any;
  beforeEach(function() {
    reader = new loadJson.JsonReader(fixture, true);
  ...
  it('deberia encontrar una version del indice', () => {
    ...
    expect(wrapper.seek("not__exits_foo", "", wrapper.haystack)).to.be.undefined;
  ...
  it("Deberia abstraer un objeto JSON con JSONType", () => {
    ...
    const jt: JSONType<string, any> = new JSONType(xjson);
